...
description: File user or operator feedback on the tasks board. Takes free text —
  title and optional detail. Files as kind=feedback, priority=P2, status=open.
...
curl -sf -X POST "${TASKS_WORKER_URL}/api/v1/tasks" \
  ...
  -d '{"venture_id":"<VENTURE_ID>","kind":"feedback","priority":"P2","status":"open","title":"<TITLE>","description":"<DESC or null>"}'
```
