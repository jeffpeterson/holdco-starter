...
description: File a new idea on the tasks board. Takes free text — title and optional
  detail. Files as kind=idea, priority=P2, status=open under this venture.
...
curl -sf -X POST "${TASKS_WORKER_URL}/api/v1/tasks" \
  ...
  -d '{"venture_id":"<VENTURE_ID>","kind":"idea","priority":"P2","status":"open","title":"<TITLE>","description":"<DESC or null>"}'
```
