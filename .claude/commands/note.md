---
description: Capture a freeform note on the tasks board. Takes free text — title and
  optional detail. Files as kind=note, no priority, status=open under the holdco venture.
argument-hint: "<title> [— optional detail]"
disable-model-invocation: true
allowed-tools: Bash
---

## Capture note: $ARGUMENTS
...
Capture the note from $ARGUMENTS as a new task on the tasks board.
...
curl -sf -X POST "${TASKS_WORKER_URL}/api/v1/tasks" \
  ...
  -d '{"venture_id":"holdco","kind":"note","status":"open","title":"<TITLE>","description":"<DESC or null>"}'
```
