---
description: File user or operator feedback on the tasks board. Takes free text —
  title and optional detail. Files as kind=feedback, priority=P2, status=open under holdco.
argument-hint: "<title> [— optional detail]"
disable-model-invocation: true
allowed-tools: Bash
---

## File feedback: $ARGUMENTS
...
File the feedback from $ARGUMENTS as a new task on the tasks board.
...
curl -sf -X POST "${TASKS_WORKER_URL}/api/v1/tasks" \
  ...
  -d '{"venture_id":"holdco","kind":"feedback","priority":"P2","status":"open","title":"<TITLE>","description":"<DESC or null>"}'
```
