---
description: File a new idea on the tasks board. Takes free text — title and optional
  detail. Files as kind=idea, priority=P2, status=open under the holdco venture.
...
File the idea from $ARGUMENTS as a new task on the tasks board.
...
curl -sf -X POST "${TASKS_WORKER_URL}/api/v1/tasks" \
  ...
  -d '{"venture_id":"holdco","kind":"idea","priority":"P2","status":"open","title":"<TITLE>","description":"<DESC or null>"}'
```
